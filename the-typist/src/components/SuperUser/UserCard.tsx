...
const UserCard = (props: any) => {
  ...
  useEffect(() => {
    if (!isEditting) {
      if (
        editUser.name !== props.user.name ||
        editUser.rol !== props.user.rol
      ) {
        API.patch(API.host, "/user/patch/", editUser)
          .then((res) => {
          ...
          <div className="section name">
            ...
            <div className="content center">
              <input
                ...
                value={editUser.name}
                onChange={(e) => {
                  handleOnChange(e, editUser.name);
                }}
                type="text"
              />
            </div>
  ...
  const renderIsAdmin = () => {
    ...
    return (
      <div className="content center flex-1">
        <button onClick={handleOnClickDelete} className="button delete">
          Delete
        </button>
        <button onClick={handleOnClickEdit} className="button edit">
  ...
  return <React.Fragment>{renderIsEditting()}</React.Fragment>;
};
